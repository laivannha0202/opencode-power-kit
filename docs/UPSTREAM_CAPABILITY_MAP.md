...
|----------|-----------|---------|----------------|-----------------|-------------------|---------------------|----------|--------------|---------------|
| Superpowers | https://github.com/obra/superpowers.git | MIT | v6.3.0 (central + template pin) | Plugin reference (loaded at runtime) | Skill system, agent delegation patterns | build-strong, all agents via skill system | N/A | Medium — runtime dependency, pinned tag | Central pin + drift validator |
| BMAD Method | https://github.com/bmad-code-org/BMAD-METHOD | MIT | 6.11.0 (CLI/update pin) | Install-time dependency | Project scaffolding, planning templates | bin/opk, update-bmad.sh, update-opk.sh | N/A | Low — install-time only | Central pin in `scripts/upstream-versions.sh` |
| GSD Core | https://github.com/open-gsd/gsd-core | MIT (npm @opengsd/gsd-core) | @1.8.0 (reviewed central pin) | Opt-in wrapper (`opk gsd`) | Workflow orchestration, agent patterns | extras/gsd-agent-reference/ (34 files, reference-only) | Active agents: no GSD in agents/ | Medium — npm dependency, pinned | Version pinned, override via env |
...
- **Verified pins**: Superpowers v6.3.0 is the current official stable release and matches the central/template pins; BMAD 6.11.0 matches its official stable release. GSD remains at the reviewed @1.8.0 central pin in this patch; the newer @1.9.1 release is intentionally not bundled into this permission/runtime fix.
