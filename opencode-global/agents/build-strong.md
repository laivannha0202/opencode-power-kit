...
1. **Spec trước khi code** — luôn tạo spec ngắn trước khi bắt đầu implementation.
2. **Slice nhỏ hơn** — mỗi slice chỉ 1 file, ≤ 50 dòng diff.
3. **Verification chặt hơn** — chạy test sau mỗi slice, không batch.
4. **Don't skip phases** — weaker models hay skip phases. PHẢI tuân thủ 7-phase pipeline.
5. **Reviewer read-only** — KHÔNG bao giờ để reviewer sửa code, chỉ comment.
