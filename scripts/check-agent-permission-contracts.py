...
MANIFEST = ROOT / "scripts" / "agent-permission-exceptions.json"
AGENT_GLOBS = (
...
def main() -> int:
    ...
    for path in agent_files():
        name = path.stem
        block = permission_block(frontmatter(path))
        ...
        if name in READ_ONLY and edit == "allow":
            read_only_edit_allow.append(name)
            errors.append(f"{path.relative_to(ROOT)}: read-only agent allows edit")

    stale = sorted(set(exceptions) - set(asks))
    for name in stale:
        errors.append(f"exception manifest contains stale agent: {name}")

    print(f"agents checked: {len(agent_files())}")
