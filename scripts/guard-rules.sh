...
#
# Design notes:
#   - No environment variable may change a block verdict to allow. The
#     guard is fail-closed: if opk_guard_scan returns dangerous, the
#     command is always blocked regardless of any env var state.
#   - NO allowlist: an allowlist is what enabled the
...
_OPK_GIT_CLEAN='(^|[^a-zA-Z0-9_])git([[:space:]]+-C[[:space:]]+[^[:space:]]+)*[[:space:]]+clean[[:space:]]+-[a-zA-Z]*[fF][a-zA-Z]*'
