...
while [ $# -gt 0 ]; do
	...
	--project-dir)
		PROJECT_DIR="${2:-}"
		[ -z "$PROJECT_DIR" ] && err "--project-dir cần tham số"
		shift
		;;
...
# --- Dispatch ---
[ "$GLOBAL_FLAG" = true ] && {
	confirm_default_yes "Cài global?"
	do_global
}
[ "$PROJECT_FLAG" = true ] && {
	confirm_default_yes "Cài project tại ${PROJECT_DIR:-$PWD_NOW}?"
	do_project
}
[ "$FULLSTACK_FLAG" = true ] && {
	confirm_default_yes "Cài fullstack tại ${PROJECT_DIR:-$PWD_NOW}?"
	do_fullstack
}
[ "$ALL_FLAG" = true ] && {
	confirm_default_yes "Cài tất cả?"
	do_all
}
